...
SUPERCELL = [10,10,10]                      # shape of the supercell
ELEMENTS = ["Sr", "Ti", "O"]                # elements in the lattice
NATS = 5                                    # number of atoms per cell

TEMPERATURES = np.linspace(270, 340, 8)       # temperatures to simulate         

cfg.MC_STEPS = 80000                        # MC total steps
...
def display_rotation_field(temps):
    ...
    for i, t in enumerate(temps):
        ...
        x, y, z = np.meshgrid(
            np.linspace(0,int(SC[0])-1, SC[0]),
            np.linspace(0,int(SC[1])-1, SC[1]),
            np.linspace(0,int(SC[2])-1, SC[2]),
        )
    
        fig = plt.figure("vector" + str(i) + ".png", figsize=[8,8])
        ax = fig.gca(projection='3d')
        
        ax.quiver(x, y, z, nu, nv, nw, length=1/np.max(SC), lw=3)
        ...
        ax.set_xticks(np.linspace(0,int(SC[0])-1, SC[0]))
        ax.set_yticks(np.linspace(0,int(SC[1])-1, SC[1]))
        ax.set_zticks(np.linspace(0,int(SC[2])-1, SC[2]))
